...
const ListNFTs = ({ marketplace, nft }) => {
  ...
  const ListERC1155 = async () => {
    if (!image || !price || !name || !description || !amount) return
    try{
      const result = await client.add(JSON.stringify({image, price, name, description, amount}))
      mintAndList(result)
    } catch(error) {
