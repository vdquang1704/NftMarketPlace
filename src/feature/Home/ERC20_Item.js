...
const Home_ERC20 = ({ marketplace, nft }) => {
 ...
 const buyMarketItem = async (item, amount) => {
  await (await marketplace.buyItemERC20(item.itemId, amount, {value: amount * item.price})).wait()
     ...
     <Table striped bordered hover responsive>
      ...
      {items.map((item, idx) => (
        <tr>
          ...
          <td>
            <Button onClick={() =>
           buyMarketItem(item, ethers.utils.parseEther(amount.toString()))}
           variant="primary" size="sm">
            Buy for {ethers.utils.formatEther
            (item.price)} ETH/token
           </Button></td>
        </tr>
